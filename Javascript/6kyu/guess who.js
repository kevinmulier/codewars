...
function GuessWho(character) {
  ...
  let myCharacter;
  for (let element of this.characteristic) {
    if (element[0] === character) {
      myCharacter = element;
    }
    availableCharacters.push(element);
  }
  let turns = 0;
  ...
  GuessWho.prototype.guess = function (guess) {
    ...
    if (guess === character) {
      return [`Correct! in ${turns} turns`];
    }

    if (myCharacter[1].includes(guess)) {
      for (let character of availableCharacters) {
        if (character[1].includes(guess)) {
          tempCharacters.push(character);
        }
      }
    } else {
      for (let character of availableCharacters) {
        if (character[0] !== guess && !character[1].includes(guess)) {
          tempCharacters.push(character);
        }
      }
    }
    availableCharacters = tempCharacters;
    return availableCharacters.map((x) => x[0]);
  };
}
