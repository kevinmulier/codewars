...
function stat(strg) {
  ...
  const times = strg.split(", ").map(toSeconds);
  const range = calculateRange(times);
...
function calculateMedian(times) {
  ...
  return sortedTimes.length % 2 !== 0 ? sortedTimes[midIndex] : Math.floor((sortedTimes[midIndex - 1] + sortedTimes[midIndex]) / 2);
}
